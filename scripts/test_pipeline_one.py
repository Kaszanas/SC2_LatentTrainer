...
Visualization: all paths projected to 2D via PCA for interpretability.

Install:
    pip install torch scikit-learn POT matplotlib numpy scipy
"""
...
INPUT_DIM = 10
LATENT_DIM = 8  # ← change freely; visualisation always uses PCA→2D
N_SAMPLES = 1000
EPOCHS = 150
BATCH_SIZE = 64
SEED = 42
...
OT_REG = 0.0  # 0 = exact EMD; >0 = Sinkhorn (faster for large N)
GRAD_STEPS = 120  # gradient ascent iterations
GRAD_LR = 0.04  # step size
GRAD_MOMENTUM = 0.85  # momentum coefficient
GRAD_DENSITY_WEIGHT = 0.3  # pull toward data manifold
GRAD_KDE_BW = 0.5  # KDE bandwidth for density regularisation
GEODESIC_K = 12  # k-NN neighbours for graph
N_WAYPOINTS = 10  # waypoints to decode for each path
...
def make_data(n=N_SAMPLES, input_dim=INPUT_DIM):
    """Two clusters with mirrored means — class-separable synthetic data."""
...
class VAEClassifier(nn.Module):
    ...
    def embed(self, x: torch.Tensor) -> torch.Tensor:
        """Deterministic μ embedding for inference."""
        mu, _ = self.encode(x)
        return mu
    ...
    def p_win(self, z: torch.Tensor) -> torch.Tensor:
        """P(win|z) — differentiable, works with or without grad."""
...
def pick_subject(model, X_test, y_test):
    ...
    xt = torch.tensor(x_raw).unsqueeze(0)
    with torch.no_grad():
        z_new = model.embed(xt.to(DEVICE)).cpu().numpy().squeeze()
        p = model.p_win(model.embed(xt.to(DEVICE))).item()
...
def path_optimal_transport(
    ...
    """
    Earth Mover's Distance from z_new (unit mass) to win distribution.
    Returns array of shape (n_waypoints, latent_dim) — straight line in latent space
    but targeted at the OT-optimal destination, not the simple centroid.
    """
    ...
    path = np.array([(1 - a) * z_new + a * z_target for a in alphas])
    return path
...
def path_gradient_ascent(
    ...
    """
    Follows ∇P(win|z) with momentum.
    A KDE density penalty keeps the trajectory on the data manifold.
    Records the full trajectory, then subsamples to n_waypoints.
    """
    # Fit KDE on all training embeddings for manifold regularisation
    kde = KernelDensity(kernel="gaussian", bandwidth=kde_bw).fit(Z_all)

    z = torch.tensor(z_new.copy(), dtype=torch.float32, requires_grad=True)
    ...
    for step in range(steps):
        ...
        with torch.no_grad():
            # Classifier gradient (ascend win prob)
            grad_cls = z.grad.clone()

            # KDE density gradient (numerical, stay on manifold)
            z_np = z.numpy()
            eps = 1e-3
            ...
            grad_density = torch.tensor(grads_kde, dtype=torch.float32)
            ...
            trajectory.append(z.detach().numpy().copy())

    trajectory = np.array(trajectory)
    # Subsample to n_waypoints
    indices = np.linspace(0, len(trajectory) - 1, n_waypoints, dtype=int)
    path = trajectory[indices]

    final_p = model.p_win(
        torch.tensor(path[-1], dtype=torch.float32).unsqueeze(0)
    ).item()
...
def path_geodesic(
    ...
    """
    Builds a k-NN graph over all training embeddings + z_new.
    Finds shortest path (Dijkstra) from z_new to the highest-density
    win region. The path is strictly on the data manifold by construction.
    """
    # KDE over win points to find dense target
    ...
    # Augment graph with z_new and best_win (if not already in Z_all)
    Z_graph = np.vstack([Z_all, z_new.reshape(1, -1), best_win.reshape(1, -1)])
    src_idx = len(Z_graph) - 2  # z_new
    tgt_idx = len(Z_graph) - 1  # best win
...
def decode_path(model, path_z: np.ndarray) -> tuple:
    """Returns (decoded_x, p_win_values)."""
    zt = torch.tensor(path_z, dtype=torch.float32)
    with torch.no_grad():
        x_hat = model.decode(zt).numpy()
        p_vals = model.p_win(zt).numpy().squeeze()
    return x_hat, p_vals
...
def fit_projections(Z_train: np.ndarray) -> dict:
    """
    Returns a dict of named callables:
        proj["PCA"](Z)  → Z_2d  (works on any new points, linear)
        proj["tSNE"](Z) → Z_2d  (re-fits; use for viz only)
        proj["UMAP"](Z) → Z_2d  (transform on fitted reducer)
    """
    projections = {}

    # PCA — deterministic, invertible, fast
    pca = PCA(n_components=2, random_state=SEED)
    pca.fit(Z_train)
    projections["PCA"] = pca.transform
    projections["_pca_obj"] = pca  # keep raw object for run_pipeline

    # t-SNE — non-parametric; we embed all points at once, then interpolate
    # For new points we use a nearest-neighbour trick: append them and re-run
    def tsne_transform(Z_query, Z_ref=Z_train, seed=SEED):
        Z_combined = np.vstack([Z_ref, Z_query])
        emb = TSNE(
            n_components=2,
            perplexity=min(30, len(Z_combined) // 4),
            random_state=seed,
            max_iter=500,
            init="pca",
            learning_rate="auto",
        ).fit_transform(Z_combined)
        return emb[len(Z_ref) :]  # return only the query rows

    # Pre-compute full training embedding so scatter is consistent
    print("  Fitting t-SNE on training embeddings …", flush=True)
    Z_combined_tsne = np.vstack([Z_train])
    tsne_full = TSNE(
        n_components=2,
    ...
    def tsne_project(Z_query):
        """Approximate: find nearest training neighbour and offset."""
        dists = np.linalg.norm(
            Z_train[:, None] - Z_query[None], axis=2
        )  # (N_train, N_query)
        nn_idx = dists.argmin(axis=0)
        return tsne_full[nn_idx]  # use nearest training point's 2D position

    projections["tSNE"] = tsne_project

    # UMAP — parametric, smooth, preserves global + local structure
    if HAS_UMAP:
...
# ══════════════════════════════════════════════════════════════
# VISUALISATION  —  one figure per projection method
# ══════════════════════════════════════════════════════════════
...
def _draw_projection_panel(
    ...
    """
    Draw a single 2D projection panel with all method paths overlaid.
    Uses density-based colouring for the cloud.
    """
    # Project training clouds
    Z_win_2d = proj_fn(Z_win_train)
    Z_loss_2d = proj_fn(Z_loss_train)

    # Colour by P(win) proxy — just class label, faded
    ...
    for name, path_z in paths.items():
        ...
        # Gradient line — draw segments with increasing alpha
        n = len(path_2d)
        for i in range(n - 1):
            alpha = 0.4 + 0.6 * (i / (n - 1))
            lw = 1.5 + 1.0 * (i / (n - 1))
        ...
        # Waypoint dots sized by index (get bigger toward target)
...
def visualise_all(
    ...
    Produces two figures:
      Figure 1 — projection grid (PCA / t-SNE / UMAP) side by side
      Figure 2 — per-method analytics (P(win) curve + feature Δ)
    """
    # ── gather active projections
    proj_items = [("PCA", projections["PCA"])]
    proj_items.append(("tSNE", projections["tSNE"]))
    ...
    for col, (name, path_z) in enumerate(paths.items()):
        ...
        bars = ax_d.bar(feat_idx, delta, color=bar_cols, alpha=0.85, width=0.7)
        ax_d.axhline(0, color="#555", lw=0.8)
        # Annotate top-3 largest absolute changes
        ...
        # ── Row 2: Waypoint heatmap — each waypoint × feature
    ...
    out2 = f"plots/outputs/{out_prefix}_analytics.png"
...
def run_pipeline(input_dim=INPUT_DIM, latent_dim=LATENT_DIM):
    ...
    # 3b. Fit projections (PCA, t-SNE, UMAP)
    print("=" * 60)
    ...
    for name, path_z in paths.items():
        ...
        dists = np.linalg.norm(np.diff(path_z, axis=0), axis=1)
        length = dists.sum()
        print(
            f"  {name.upper():<6}  {p_vals[0]:<14.3f}  {p_vals[-1]:<12.3f}  {length:.4f}"
        )

    # 6. Visualise — projections + analytics
    print()
    print("=" * 60)
...
if __name__ == "__main__":
    # ← Change these two numbers freely
