...
def plot_losses(data, output_path):
    ...
    loss_tags = [t for t in data if 'loss' in t.lower() and ('epoch' in t.lower() or t.startswith('val_'))]
    acc_tags = [t for t in data if 'acc' in t.lower() and ('epoch' in t.lower() or t.startswith('val_'))]
    ...
    n_plots = 0
    if loss_tags: n_plots += 1
    if acc_tags: n_plots += 1
    if n_plots == 0:
    ...
    if loss_tags:
        ...
        for tag in sorted(loss_tags):
            d = data[tag]
            label = tag.replace('_epoch', '').replace('train_', 'train ').replace('val_', 'val ')
...
if __name__ == '__main__':
    ...
    if data:
        ...
        for tag, d in sorted(data.items()):
            vals = d['values']
            print(f"{tag}: {len(vals)} points, range [{min(vals):.4g}, {max(vals):.4g}]")
