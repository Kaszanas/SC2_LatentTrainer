...
def preprocess_dataset(
    ...
    print(f"  Total replays: {total} (train: {len(train_dataset)}, val: {len(val_dataset)})")
    ...
    for i in tqdm(range(len(train_dataset)), desc="  Train"):
        try:
            ...
            features, label = result
            train_features.append(features if isinstance(features, torch.Tensor) else torch.tensor(features, dtype=torch.float32))
    ...
    for i in tqdm(range(len(val_dataset)), desc="  Val"):
        try:
            ...
            features, label = result
            val_features.append(features if isinstance(features, torch.Tensor) else torch.tensor(features, dtype=torch.float32))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--transform", choices=["rich", "economy"], default="rich",
    ...
    preprocess_dataset(output_path=output_path, transform_fn=transform_fn, transform_name=args.transform)
