...
class UnsupervisedGuidedVAE(nn.Module):
    def __init__(self, n_latent_dimensions=10, dimensions=64):
        super().__init__()
        self.DIM = dimensions
        ...
        self.layer41 = nn.Linear(4 * 4 * 4 * dimensions, n_latent_dimensions)
        self.layer42 = nn.Linear(4 * 4 * 4 * dimensions, n_latent_dimensions)

        self.preprocess = nn.Sequential(
            nn.Linear(n_latent_dimensions, 4 * 4 * 4 * dimensions),
            nn.ReLU(True),
        )
        ...
        self.fc1 = nn.Linear(n_latent_dimensions - 2, 784)
    ...
    def reparameterize(self, mu: int, logvar: int):
