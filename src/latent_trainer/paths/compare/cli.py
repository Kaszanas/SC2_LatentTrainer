...
def _print_summary(stats) -> None:
    ...
    print(
        f"  SD-adjusted threshold: {threshold:.3f}  (0.5 + σ, σ = {threshold - 0.5:.3f})"
    )
    header = (
    ...
    for s in stats:
        ...
        cond = (
            f"{s.cond_success_rate:.1%}" if not math.isnan(s.cond_success_rate) else "—"
        )
        sd_adj = (
            f"{s.sd_adj_success_rate:.1%}"
