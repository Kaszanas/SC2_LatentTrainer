...
def main():
    ...
    train_X, train_y, val_X, val_y, _, _ = load_and_normalize("data/cached_dataset_rich.pt")
    ...
    for idx, (title, data) in enumerate(views):
        ...
        ax.scatter(coords[lose_mask, 0], coords[lose_mask, 1], 
                   c='#e74c3c', alpha=0.3, s=8, label=f'Loss ({lose_mask.sum()})')
        ax.scatter(coords[win_mask, 0], coords[win_mask, 1], 
                   c='#2ecc71', alpha=0.3, s=8, label=f'Win ({win_mask.sum()})')
    ...
    ax2.scatter(coords[lose_mask, 0], coords[lose_mask, 1],
                c='#e74c3c', alpha=0.3, s=12, label=f'Player 1 Loss ({lose_mask.sum()})')
    ax2.scatter(coords[win_mask, 0], coords[win_mask, 1],
                c='#2ecc71', alpha=0.3, s=12, label=f'Player 1 Win ({win_mask.sum()})')
    
    ax2.set_title("t-SNE — Concatenated Latent Space (Classifier Input)", fontsize=14, fontweight='bold')
