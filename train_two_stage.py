...
class SimpleVAE(nn.Module):
    def __init__(self, input_dim=203, latent_dim=32):
        super().__init__()
...
def train_vae_stage(train_X, val_X, latent_dim, epochs, batch_size, lr, device):
    ...
    train_loader = DataLoader(TensorDataset(train_flat), batch_size=batch_size, shuffle=True)
    val_loader = DataLoader(TensorDataset(val_flat), batch_size=batch_size, shuffle=False)
    ...
    print(f"\n{'='*60}")
    print(f"STAGE 1: VAE Reconstruction Training")
...
def extract_latents(vae, data, device, batch_size=256):
    ...
    for player_idx in range(2):
        ...
        with torch.no_grad():
            for i in range(0, len(player_data), batch_size):
                ...
                mu, _ = vae.encode(batch)  # Use mean (not sampled z) for deterministic encoding
...
def train_classifier_stage(train_z, train_y, val_z, val_y, latent_dim, epochs, batch_size, lr, device):
    ...
    print(f"\n{'='*60}")
    print(f"STAGE 2: Classifier on Frozen Latent Space")
...
def main():
    ...
    parser.add_argument("--cache", default="data/cached_dataset_rich.pt", help="Path to cached dataset")
    parser.add_argument("--latent-dim", type=int, default=32, help="VAE latent dimension")
    parser.add_argument("--vae-epochs", type=int, default=200, help="Max epochs for VAE stage")
    parser.add_argument("--cls-epochs", type=int, default=100, help="Max epochs for classifier stage")
    parser.add_argument("--batch-size", type=int, default=256, help="Batch size")
    parser.add_argument("--vae-lr", type=float, default=1e-3, help="VAE learning rate")
    parser.add_argument("--cls-lr", type=float, default=1e-3, help="Classifier learning rate")
    ...
    print(f"  Model saved to:      output/two_stage_model.pth")
    print(f"{'='*60}")
